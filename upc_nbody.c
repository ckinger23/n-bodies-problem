 ...
 *  Description:
 *
 *    This program uses to UPC to implement SPMD programming in order to 
 *    run a timestep solution on the N-Bodies problem. In this problem,
 *    there are bodies that represent planets with their own 3D position, 
 *    velocity, acceleration, and mass. Each planet affects all other 
 *    planets in relation to these attributes.
 */
...
//variables to use in algorithm
#define N 10000
#define G 6.67e-11
#define TIMESTEP 0.25
#define NSTEPS 10
//an array of the particular threads
...
// The array of bodies at time t
shared body_t bodies[N];
//the array of bodies at time t + 1
shared body_t next[N];
...
void computeforce(shared body_t *me, shared body_t *nextme) {
...
void print_body(shared body_t *b) {
...
int main(int argc, char **argv) {
  ...
  for(int ts = 0; ts < NSTEPS; ts++) {
    ts_start = get_wctime();
    //compute the attributes of each body in relation to all other bodies
